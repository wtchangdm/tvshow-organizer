...
const seasonPicker = /S(\d{2})/i;
const episodePicker = /E(\d{2})/i;
  ...
  if (shouldSkipConverting) {
    ...
    await new Promise(resolve => {
      ...
      conversionProcess.stdout.on("data", data => {
        // FFMpeg's output doesn't go here. Weird.
        const msg = data.toString("utf-8");
        if (msg) console.log(`[${filename}] ${msg}`);
      });
      conversionProcess.stderr.on("data", data => {
        // Skip when data[0] === 13 (data.length === 1), which is CR is ASCII.
        if (data[0] !== 13) console.log(`[${filename}] ${data.toString()}`);
      });
